...
*/
    ...
    eci2kep(cosmos_data->node.loc.pos.eci, kep);
	if (!dp)
	{
		dp = dt;
		tp = kep.period;
...
//	cosmos_data->node.loc.pos.geod.s.lat;
	...
	do
	{
        ...
        jplpos(JPL_EARTH,JPL_MOON,utc2tt(cosmos_data->node.utc),&ipos);
...
//		calc_hardware_torque(cosmos_data->node.loc,torque,&rtorque1,&rtorque2,&mtorque,&ralp,&mtrx,&mtry,&mtrz,cosmos_data);
        ...
        bearth =  drotate(cosmos_data->node.loc.att.icrf.s,cosmos_data->node.loc.bearth);
