...
//	cosmos_data->sdata.node.loc.pos.geod.s.lat;
    ...
    cosmos_data->sdata.physics.moi[0] = 2.3;
    cosmos_data->sdata.physics.moi[1] = 2.6;
    cosmos_data->sdata.physics.moi[2] = 2.7;
	tpointer = tstring;
...
//		calc_hardware_torque(cosmos_data->sdata.node.loc,torque,&rtorque1,&rtorque2,&mtorque,&ralp,&mtrx,&mtry,&mtrz,cosmos_data);
		...
		if (cp >= dp)
		{
            ...
            printf("%.10g\t%.10g\t%.10g\t",cosmos_data->pdata.physics.ftorque[0],cosmos_data->pdata.physics.ftorque[1],cosmos_data->pdata.physics.ftorque[2]);
