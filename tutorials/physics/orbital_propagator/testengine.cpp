...
int main(int argc, char *argv[])
{
    ...
    Convert::cartpos ipos;
    Convert::locstruc tloc, iloc;
    double localtime;
    double dt, dp, maxr, minr, mjdnow, mjdbase, mjdlast, tp, cp;
    double radius;
    double deltas, deltav;
    double rwomg;
    double ralp=0., mtrx=0., mtry=0., mtrz=0.;
    uint32_t order = 8;
    ...
    cosmos_data->node.loc.att.icrf.v = rv_smult(1.,rv_unitz());
    cosmos_data->node.loc.att.icrf.s = q_eye();
    cosmos_data->node.loc.att.icrf.a = rv_zero();

    // STK: CDR
    // STK: a(6578km),e(0deg),i(45deg),raan(0deg),ea(0deg),mm()
    strcpy(stkname,"stk_cdr_j2000.txt");
    if (Convert::load_stk(stkname, stk)<2)
        exit(1);

    // STK
    mjdbase = mjdnow = stk.pos[1].utc;
    if ((iretn=Convert::stk2eci(mjdnow, stk, ipos)) < 0)
    {
...
Physics::gauss_jackson_init_tle(order,dt,tle.mjd,tle,cosmos_data);
line2eci(cosmos_data->node.mjd,&tle,&ipos);
...
*/
    ...
    Convert::eci2kep(cosmos_data->node.loc.pos.eci, kep);
    if (!dp)
    {
        dp = dt;
        tp = kep.period;
    ...
    do
    {
        ...
        Physics::gauss_jackson_propagate(gjh, cosmos_data->node.phys, cosmos_data->node.loc, mjdnow);

        tloc = cosmos_data->node.loc;
        tloc.att.lvlh.s = q_eye();
        tloc.att.lvlh.v = rv_zero();
        Convert::att_lvlh2icrf(&tloc);
        Convert::att_icrf2lvlh(&tloc);
        Convert::att_lvlh2icrf(&tloc);

        //		torque = calc_control_torque(dt,tloc,cosmos_data);
        //		calc_hardware_torque(cosmos_data->node.loc,torque,&rtorque1,&rtorque2,&mtorque,&ralp,&mtrx,&mtry,&mtrz,cosmos_data);
        ftorque = rv_add(rtorque1,rv_add(rtorque2,mtorque));
        ftorque = drotate(q_conjugate(cosmos_data->node.loc.att.icrf.s),ftorque);
        ...
        rtorque1 = drotate(q_conjugate(cosmos_data->node.loc.att.icrf.s),rtorque1);
        ...
        {
            cp = 0;
            deltas = length_rv(rv_quaternion2axis(q_fmult(tloc.att.icrf.s,q_conjugate(cosmos_data->node.loc.att.icrf.s))));
            if (deltas > DPI)
        ...
        //		mjdnext = currentmjd(cosmos_data->node.utcoffset);
        if (cosmos_data->node.loc.pos.geos.s.r < minr)
            minr = cosmos_data->node.loc.pos.geos.s.r;
        if (cosmos_data->node.loc.pos.geos.s.r > maxr)
            maxr= cosmos_data->node.loc.pos.geos.s.r;
    } while (mjdnow < mjdlast && cosmos_data->node.loc.pos.geod.s.h > 8600.);
    printf("\n");
}
