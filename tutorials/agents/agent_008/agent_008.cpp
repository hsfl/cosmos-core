...
//!
//! \brief agent_008 is a test agent that demonstrates the interconnectivity with another agent, namely agent_002, through the use of agent requests.
...
int main(int, char **)
{
    ...
    string agentname = "008"; // Forward facing name of the agent
    string nodename = "cubesat1"; // The node that the agent will run on
    string agent004 = "004"; // The name of the agent this agent will speak to
    ...
    beatstruc beat_agent_004 = agent->find_server(nodename, agent004, 2.);
    ...
    while(agent->running())
    {
        ...
        else
        {
            ...
            cout << "beat agent_004 node: " << beat_agent_004.utc << endl;
        }
