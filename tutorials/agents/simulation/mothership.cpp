...
// Sample request function prototype
...
int main(int argc, char **argv)
{
    ...
    while (agent->running()) {
		...
		// sample update_sim_state(..) function
		double t = currentmjd();
		c->update_sim_state(t);

		// output state
		cout<<"object-oriented ways to output the states information in JSON..."<<endl;
		cout<<c->get_json_pretty("sim_states")<<endl;
		cout<<c->get_json("sim_states")<<endl;
		cout<<c->get_json_pretty("sim_states[3]")<<">"<<endl;
		cout<<"x position = <"<<c->get_json_pretty("sim_states[3].x_pos")<<">"<<endl;
		// etc...
...
//  ALLISON
		...
		// ask for their state
		response.clear(); agent->send_request(agent->find_agent("daughter_01", "allison", 2.), "get_state", response, 2.);
		// if daughter found
		if(response.size())	{
...
//  BECKY
		// ask for their state
		response.clear(); agent->send_request(agent->find_agent("daughter_02", "becky", 2.), "get_state", response, 2.);
		// if daughter found
		if(response.size())	{
...
//  CECILIA
		// ask for their state
		response.clear(); agent->send_request(agent->find_agent("daughter_03", "cecilia", 2.), "get_state", response, 2.);
		// if daughter found
		if(response.size())	{
...
//  DELILAH
		// ask for their state
		response.clear(); agent->send_request(agent->find_agent("daughter_04", "delilah", 2.), "get_state", response, 2.);
		// if daughter found
		if(response.size())	{
		...
		// And now you can can HCL and MAC ...
		// Using the following method for modifying sim_states via python (vi JSON strings)


		// from the function "modify_sim_states" in the python script "testscript.py"
		// (which should be in the same directory as this executable)
		...
		cout<<"before x_pos = "<<c->sim_states[0].x_pos<<endl;


		// call a function in python
		// input:	JSON formatted string of vector<sim_state>
		// output:	JSON formatted string of vector<sim_state>
		string modified_sim_states = modify_sim_states(c->get_json("sim_states")).cast<string>();

		// update the STATE in Namespace
		c->from_json(modified_sim_states);

		cout<<"after  x_pos = "<<c->sim_states[0].x_pos<<endl;
