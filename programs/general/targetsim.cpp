...
//    initialloc.att.icrf.s = q_eye();
    ...
    {
        sit->second->AddTarget(target.name, target.clat + target.size / 2., clon - target.size / (2. * cos(target.clat)), target.clat - target.size / 2., target.clon + target.size / (2. * cos(target.clat)), target.alt);
    }
