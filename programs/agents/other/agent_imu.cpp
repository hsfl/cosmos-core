...
// IMU Specific Info
// *********************************************************************
//#define ACCELX  (agent->cinfo->pdata.devspec.imu[0]->accel.col[0])
//#define ACCELY  (agent->cinfo->pdata.devspec.imu[0]->accel.col[1])
//#define ACCELZ  (agent->cinfo->pdata.devspec.imu[0]->accel.col[2])
//#define OX      (agent->cinfo->pdata.devspec.imu[0]->omega.col[0])
//#define OY      (agent->cinfo->pdata.devspec.imu[0]->omega.col[1])
//#define OZ      (agent->cinfo->pdata.devspec.imu[0]->omega.col[2])
//#define MAGX    (agent->cinfo->pdata.devspec.imu[0]->mag.col[0])
//#define MAGY    (agent->cinfo->pdata.devspec.imu[0]->mag.col[1])
//#define MAGZ    (agent->cinfo->pdata.devspec.imu[0]->mag.col[2])
...
LsFit lastimuomega(4, 2);
LsFit lastimumag(4, 2);
...
int main(int argc, char *argv[])
{
    ...
    // Add Requests
    if ((iretn=agent->add_request("mag",request_mag,"Magnetic Field Data Only\n\t\tRespFormat: MAGxyz(+mag)[uT],MJD")))
        exit (iretn);
    if ((iretn=agent->add_request("all",request_all,"All IMU Data\n\t\tRespFormat: MAGxyz(+mag)[uT],Accel[g]xyz,AngRatesXYZ[r/s],MJD")))
        exit (iretn);
    if ((iretn=agent->add_request("fltdata",request_imufltdata,"Only IMU Flight Data\n\t\tRespFormat: MAG[uT]xyz,AngRates[r/s]xyz,MJD")))
        exit (iretn);
    if ((iretn=agent->add_request("debug",request_debug,"IMU Debug information")))
        exit (iretn);
    ...
    std::string cnodedir = get_nodedir(agent->cinfo->pdata.node.name);
...
void run_imu_handle_closed()
{
    ...
    iretn = vn100_connect(agent->cinfo->pdata.port[agent->cinfo->pdata.devspec.imu[0]->gen.portidx].name, &vn100handle);
    IMU_MUTEX_UNLOCK;
    if (iretn < 0)
    {
        // Fail!
...
void imu_state_to_open()
{
    ...
    // Flag COSMOS Active
    agent->cinfo->pdata.devspec.imu[0]->gen.flag = DEVICE_FLAG_ON;
    agent->cinfo->pdata.devspec.imu[0]->gen.flag |= DEVICE_FLAG_ACTIVE;
...
void imu_state_to_closed()
{
    ...
    agent->cinfo->pdata.devspec.imu[0]->gen.flag = DEVICE_FLAG_OFF;
    agent->cinfo->pdata.devspec.imu[0]->gen.flag &= ~DEVICE_FLAG_ACTIVE;
...
int32_t imu_init_data()
{
    ...
    // IMU cinfo
    //newimu = *(agent->cinfo->pdata.devspec.imu[0]);
...
int32_t imu_take_measurement()
{
    ...
    if ((iretn=vn100_measurements(&vn100handle)) >= 0)      // 20150805JC: @~10Hz Runs, This line takes up about 6.1% CPU, rest of program takes about 1.3% CPU
    {
        ...
        agent->cinfo->pdata.devspec.imu[0]->gen.utc = MEAS_TIMESTAMP;
        ...
        *(agent->cinfo->pdata.devspec.imu[0]) = newimu;
        //        agent->cinfo->pdata.devspec.imu[0]->omega = newimu.omega;
        //        agent->cinfo->pdata.devspec.imu[0]->mag = newimu.mag;
        //        agent->cinfo->pdata.devspec.imu[0]->accel = newimu.accel;
...
{
    IMU_MUTEX_LOCK;
    double mjd = agent->cinfo->pdata.devspec.imu[0]->gen.utc;
...
int32_t imu_post_to_broadcast()
{
    ...
    // 0xBE for IMU
    iretn = agent->post(0xBE, imuDataString);
