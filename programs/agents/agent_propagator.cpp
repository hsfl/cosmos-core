    ...
    if (fp != nullptr)
    {
        ...
        while (fgets((char *)line.data(), 200, fp) != nullptr)
        {
            ...
            for (Physics::Simulator::StateList::iterator sit = sim->cnodes.begin(); sit != sim->cnodes.end(); sit++)
            {
                if (args.size() == 4)
                {
                    sim->AddTarget(args[0], RADOF(stod(args[1])), RADOF(stod(args[2])), 0., stod(args[3]), NODE_TYPE_GROUNDSTATION);
                }
                else if (args.size() == 5)
                {
                    sim->AddTarget(args[0], RADOF(stod(args[1])), RADOF(stod(args[2])), stod(args[3]), stod(args[4]), NODE_TYPE_TARGET);
                }
            }
        }
    }
...
int32_t parse_sat(string args)
{
    ...
    if (!jargs["detector"].is_null())
    {
        ...
        camstruc det;
        det.name = values["name"].string_value();
        if (det.name == "")
        {
            det.name = "cam" + to_unsigned(sim->detectors.size(), 2, true);
        }
        ...
        det.state = 0;
        sim->AddDetector(det);
        dets.push_back(sim->detectors[det.name]);
    }
...
int32_t parse_target(string args)
{
    ...
    for (Physics::Simulator::StateList::iterator sit = sim->cnodes.begin(); sit != sim->cnodes.end(); sit++)
    {
        sim->AddTarget(name, clat+dlat, clon-dlon, clat-dlat, clon+dlon, alt, type);
    }
