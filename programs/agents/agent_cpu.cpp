...
int32_t request_diskpercent (char*request, char *response, void *cdata);
...
char sohstring[] = "{\"device_cpu_disk_000\",\"device_cpu_maxdisk_000\",\"device_cpu_maxmem_000\",\"device_cpu_mem_000\",\"device_cpu_load_000\",\"}";
...
int main(int argc, char *argv[])
{
	...
	nodename = devicename + "_node"; // append _node to name
#else
	char devicename[128];
	gethostname(devicename, sizeof devicename);
	strcat (devicename, "_node"); // append _node to name
	nodename = devicename;
	...
	{
		printf("Couldn't find Node directory, making directory now... %s\n",argv[1]);
#ifdef COSMOS_WIN_OS
	...
	if ((iretn=agent_add_request(cdata, (char *)"load",request_load)))
		exit (iretn);
	if ((iretn=agent_add_request(cdata, (char *)"disk_percent",request_diskpercent)))
		exit (iretn);
...
int myagent()
{
	...
	while(agent_running(cdata))
	{
		...
		cdata[0].devspec.cpu[0]->maxdisk = GetLinuxTotalDisk();
...
double GetLinuxTotalDisk() // NOT TESTED
{
	struct statvfs buf;
	int64_t disk_size, used, free;
	char *path = (char*) "/";
	...
	return (free+used) * 0.000976563; // convert byte to kibibyte
}
...
int32_t request_diskpercent (char *request, char *response, void *)
{
	...
	string disk = to_string(diskpercent);
#else
	float diskpercent = GetLinuxUsedDisk() / GetLinuxTotalDisk();
	string disk = to_string(diskpercent);
#endif
	return (sprintf(response, "%s", disk.c_str()));
}
