...
int main(int argc, char *argv[])
{
    ...
    if (iretn < 0)
    {
        agent->debug_log.Printf("Failed to add CPU %s\n", cosmos_error_string(iretn).c_str());
        agent->shutdown();
        exit(1);
    }
    ...
    agent->debug_log.Printf("CPU Agent initialized\n");
    ...
    while(agent->running())
    {
        ...
        {
            agent->debug_log.Printf("%16.10f ", agent->cinfo->device[cpu_cidx]->utc);
        }
        ...
        if (agent->get_debug_level())
        {
            agent->debug_log.Printf("Load %6.2f %6.2f ", agent->cinfo->devspec.cpu[cpu_didx].load, agent->cinfo->devspec.cpu[cpu_didx].maxload);
            agent->debug_log.Printf("Memory %6.2f %6.2f ", agent->cinfo->devspec.cpu[cpu_didx].gib, agent->cinfo->devspec.cpu[cpu_didx].maxgib);
        }
        ...
        for (size_t i=0; i<agent->cinfo->devspec.disk_cnt; ++i)
        {
            ...
            if (agent->get_debug_level())
            {
                agent->debug_log.Printf("%s %6.2f %6.2f ", node_path.c_str(), agent->cinfo->devspec.disk[i].gib, agent->cinfo->devspec.disk[i].maxgib);
            }
        }
