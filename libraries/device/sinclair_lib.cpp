                ...
                if (crc == crc2) // CRCs match?
                {
                    ...
                    if ((handle->mbuf.mcf&0x1f) == NSP_COMMAND_COMBINATION || (handle->mbuf.mcf&0x1f) == NSP_COMMAND_READ_RESULT)
                    {
                        ...
                        place = uint16from(&xbuf[3],ORDER_LITTLEENDIAN);
                         fflush(stdout);
                        if (place < NSP_MAX_MESSAGE)
                        {
                            size_t nbytes = iretn - 7;
                            if (place + nbytes >= NSP_MAX_MESSAGE)
                            {
                                nbytes -= place + nbytes - NSP_MAX_MESSAGE;
                            }
                            if (nbytes > 0)
                            {
//                                printf("Copy to place: %u - %u\n", place, place + nbytes - 1);
                                memcpy((void *)&handle->mbuf.data[place], (void *)&xbuf[5], iretn-7);
                                handle->mbuf.size = place + nbytes;
                            }
                        }
                    }
                    else
                    {
                        printf("Copy to size: %u\n", handle->mbuf.size);
                        fflush(stdout);
