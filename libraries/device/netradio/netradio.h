...
namespace Cosmos {
    namespace Devices {
        ...
        public:
            ...
            int32_t Queue(queue<PacketComm> &queue, std::mutex &mtx, const PacketComm &p);
            int32_t DeQueue(queue<PacketComm> &queue, std::mutex &mtx, PacketComm &p);
            int32_t PacketIn(PacketComm &p);
            int32_t PacketInSize();
            int32_t PacketOut(PacketComm &p);
            int32_t PacketOutSize();
            int32_t Clear(queue<PacketComm> &queue, std::mutex &mtx);
            int32_t Init();
            void Join();
