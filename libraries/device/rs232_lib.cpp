...
char comports[30][30]={"/dev/tty.usbmodem1411","/dev/ttyS1","/dev/ttyS2","/dev/ttyS3","/dev/ttyS4","/dev/ttyS5",
                       "/dev/ttyS6","/dev/ttyS7","/dev/ttyS8","/dev/ttyS9","/dev/ttyS10","/dev/ttyS11",
                       "/dev/ttyS12","/dev/ttyS13","/dev/ttyS14","/dev/ttyS15","/dev/ttyUSB0",
                       "/dev/ttyUSB1","/dev/ttyUSB2","/dev/ttyUSB3","/dev/ttyUSB4","/dev/ttyUSB5",
                       "/dev/ttyAMA0","/dev/ttyAMA1","/dev/ttyACM0","/dev/ttyACM1",
...
/*    case  460800 : baudr = B460800;
  ...
  // original settings don't work for mac
  /*
  new_port_settings.c_cflag = baudr | CS8 | CLOCAL | CREAD;
  new_port_settings.c_iflag = IGNPAR;
  new_port_settings.c_oflag = 0;
 ...
 */
  ...
  new_port_settings.c_iflag &= ~(IXON | IXOFF | IXANY); // turn off s/w flow ctrl

  new_port_settings.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG); // make raw
  new_port_settings.c_oflag &= ~OPOST; // make raw
...
int RS232_SendBuf_Block_Timeout(int comport_number, const unsigned char *text, unsigned int txcount)
{
	// 20130507JC: Written to address local Linux serial port buffer overflow problems
	unsigned int retry_max;
	unsigned int retry_count;
	unsigned int retry_udelay;
	unsigned int position;
	int32_t iretn;

	// Retry Max 10000 times with 100us delay = 1s retry
	// At 1s, 19200bps = 1920 max buffer
	retry_max = 10000;
	retry_udelay = 100;
	...
	while(position != txcount && retry_count < retry_max)
	{
		iretn = RS232_SendBuf(comport_number,(unsigned char *)(text+position),(txcount-position));
		if (iretn > 0) position += iretn;
		if (position != txcount && retry_count < retry_max)
		{
			retry_count++;
			COSMOS_USLEEP(retry_udelay);
		}
	}

	return position;  // Returns number of bytes successfully sent
}
