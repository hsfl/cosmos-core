...
{   uint16_t fcs;
	...
	fcs = 0xffff;
	while (length--)
    {   fcs = (fcs >> 8) ^ kisstnc_fcstab[(fcs ^ ((uint16_t)*data)) & 0xff];
		data++;
	}
    return (~fcs);
}
...
{
	cssl_start();
    handle->serial = cssl_open(dev,KISSTNC_BAUD, KISSTNC_BITS,KISSTNC_PARITY,KISSTNC_STOPBITS);
...
int32_t kisstnc_recvframe(kisstnc_handle *handle)
{
    ...
    iretn = cssl_getslip(handle->serial, handle->frame.full, KISSTNC_FULL_SIZE);
	if (iretn >= 0)
	{
//		int16_t ch = cssl_getchar(handle->serial);
...
int32_t kisstnc_loadframe(kisstnc_handle *handle, std::vector<uint8_t> data)
{
    ...
    tsize = (data.size() <= KISSTNC_PAYLOAD_SIZE?data.size():KISSTNC_PAYLOAD_SIZE);
    handle->frame.size = tsize + 17; // Modified to +17 from +19 (FCS no longer needed)
	memcpy(handle->frame.part.payload, &data[0], tsize);

//    fcs = kisstnc_calc_fcs(&data[0], tsize);
//	handle->frame.part.payload[tsize] = fcs%256;
//	handle->frame.part.payload[tsize+1] = fcs>>8;
	//	handle->frame.part.payload[tsize+2] = AX25_FLAG;
...
int32_t kisstnc_unloadframe(kisstnc_handle *handle, uint8_t *data, uint16_t size)
{
	...
	tsize = (handle->frame.size-17 <= size?handle->frame.size-17:size);
