...
int pt1000_image(gige_handle *handle, uint32_t frames, gige_data &data)
{
    ...
    ElapsedTime et;
    double telapsed = frames * 4. * handle->bufferout.size() / handle->streambps;
    while (et.split() < telapsed)
    {
        if ((nbytes=recvfrom(handle->stream.cudp, handle->bufferin.data() ,handle->bestsize, 0, static_cast<struct sockaddr *>(nullptr),static_cast<socklen_t *>(nullptr))) > 0)
        {
    ...
    {
        tbytes = 0;
        while (tbytes < handle->bufferout.size()*2 && et.split() < telapsed)
        {
            if ((nbytes=recvfrom(handle->stream.cudp, handle->bufferin.data() ,handle->bestsize, 0, static_cast<struct sockaddr *>(nullptr),static_cast<socklen_t *>(nullptr))) > 0)
            {
                switch (handle->bufferin[4])
                {
                case 1:
                    break;
                case 2:
                    break;
                case 3:
                    memcpy(reinterpret_cast<uint8_t *>(handle->bufferout.data()) + tbytes, &handle->bufferin[8], nbytes-8);
                    tbytes += nbytes-8;
                    break;
                }
            }
