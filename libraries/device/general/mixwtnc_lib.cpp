...
#include "device/general/mixwtnc_lib.h"
...
{   uint16_t fcs;
	...
	fcs = 0xffff;
	while (length--)
    {   fcs = (fcs >> 8) ^ mixw_fcstab[(fcs ^ ((uint16_t)*data)) & 0xff];
		data++;
	}
    return (~fcs);
}
...
{
	cssl_start();
    handle->serial = cssl_open(dev,MIXWTNC_BAUD, MIXWTNC_BITS,MIXWTNC_PARITY,MIXWTNC_STOPBITS);
...
int32_t mixwtnc_loadframe(mixwtnc_handle *handle, uint8_t *data, uint16_t size)
{
	...
	for (uint16_t i=0; i<7; ++i)
	{
		...
		handle->frame.part.header.source_address[i] <<= 1;
	}
    tsize = (size <= MIXWTNC_PAYLOAD_SIZE?size:MIXWTNC_PAYLOAD_SIZE);
    handle->frame.size = tsize + 17; // Modified to +17 from +19 (FCS no longer needed)
	memcpy(handle->frame.part.payload, data, tsize);

    // No FCS required for MixW, MIXW TNC generates outgoing FCS
    //    fcs = mixwtnc_calc_fcs(data, tsize);
    //	handle->frame.part.payload[tsize] = fcs%256;
    //	handle->frame.part.payload[tsize+1] = fcs>>8;
    //	handle->frame.part.payload[tsize+2] = AX25_FLAG;
...
int32_t mixwtnc_loadframe(mixwtnc_handle *handle, vector<uint8_t> data)
{
    ...
    tsize = (data.size() <= MIXWTNC_PAYLOAD_SIZE?data.size():MIXWTNC_PAYLOAD_SIZE);
    handle->frame.size = tsize + 17; // Modified to +17 from +19 (FCS no longer needed)
	memcpy(handle->frame.part.payload, &data[0], tsize);

    // No FCS required for MixW, MIXW TNC generates outgoing FCS
    //    fcs = mixwtnc_calc_fcs(&data[0], tsize);
    //	handle->frame.part.payload[tsize] = fcs%256;
    //	handle->frame.part.payload[tsize+1] = fcs>>8;
    //	handle->frame.part.payload[tsize+2] = AX25_FLAG;
...
int32_t mixwtnc_unloadframe(mixwtnc_handle *handle, uint8_t *data, uint16_t maxsize)
{
	uint16_t tsize;

    tsize = (handle->frame.size-17 <= maxsize?handle->frame.size-17:maxsize);
	memcpy(data, handle->frame.part.payload, tsize);
	data[tsize] = 0;
	return (tsize);
}
...
int32_t mixwtnc_unloadframe(mixwtnc_handle *handle, vector<uint8_t>& data)
{
    ...
    if (handle->frame.size >= 17)
	{
        tsize = handle->frame.size-17;
        data.resize(tsize);
		memcpy(&data[0], handle->frame.part.payload, tsize);
	}
	else
	{
		tsize = 0;
...
int32_t mixwtnc_unloadframe(mixwtnc_frame *frame, vector<uint8_t>& data)
{
    ...
    if (frame->size >= 17)
    {
        tsize = frame->size-17;
        data.resize(tsize);
        memcpy(&data[0], frame->part.payload, tsize);
    }
    else
    {
        tsize = 0;
