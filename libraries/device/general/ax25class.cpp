...
//    for(i=18; input[i] != SLIP_FEND; i++)	// FEND = FEND
//	{
...
//	for(;j <= PACKETMAX;)
//	{
//     if(intermediate_packet[j] == SLIP_FEND)
//		 return i;

//     if(intermediate_packet[j] == SLIP_FESC) //if FESC
//	 {
//         if(intermediate_packet[j+1] == SLIP_TFEND) // Check for TFEND || intermediate_packet[j+1] == SLIP_TFESC) //if transp
//		 {
//            intermediate_packet[j] = SLIP_FEND;
//            unload_packetd_payload[i] = intermediate_packet[j];
//			j+=2;
//			i++;
//		 }
//         else if(intermediate_packet[j+1] == SLIP_TFESC)	// Check for TFESC
//		 {
//            intermediate_packet[j] = SLIP_FESC;
//            unload_packetd_payload[i] = intermediate_packet[j];
//			j+=2;
//			i++;
//		 }
