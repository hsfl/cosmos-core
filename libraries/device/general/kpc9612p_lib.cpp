...
{   uint16_t fcs;
    ...
    fcs = 0xffff;
    while (length--)
    {   fcs = (fcs >> 8) ^ kpc9612p_fcstab[(fcs ^ ((uint16_t)*data)) & 0xff];
        data++;
    }
    return (~fcs);
}
...
int32_t kpc9612p_loadframe(kpc9612p_handle *handle, uint8_t *data, uint16_t size)
{
    ...
    tsize = (size <= KPC9612P_PAYLOAD_SIZE?size:KPC9612P_PAYLOAD_SIZE);
    //	handle->frame.size = tsize + 19;
    handle->frame.size = tsize + 17;
    memcpy(handle->frame.part.payload, data, tsize);
    //	fcs = kpc9612p_calc_fcs(handle->frame.full, handle->frame.size);
...
int32_t kpc9612p_loadframe(kpc9612p_handle *handle, std::vector<uint8_t> data)
{
    ...
    tsize = (data.size() <= KPC9612P_PAYLOAD_SIZE?data.size():KPC9612P_PAYLOAD_SIZE);
    //	handle->frame.size = tsize + 19;
    handle->frame.size = tsize + 17;
