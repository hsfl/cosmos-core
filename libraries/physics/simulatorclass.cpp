...
namespace Cosmos
{
    namespace Physics
    {
         ...
         */
        int32_t Simulator::AddNode(string nodename, Structure::Type stype, Propagator::Type ptype, Propagator::Type atype, Propagator::Type ttype, Propagator::Type etype, Propagator::Type oeventtype, Convert::cartpos lvlh, Convert::cartpos origineci, Convert::qatt icrf, uint8_t propagation_priority)
        {
            auto it = AddNode(nodename, propagation_priority);
            error = (*it)->Init(nodename, dt, stype, ptype, atype, ttype, etype, oeventtype, origineci, icrf);
            if (error < 0)
            {
                return error;
            }
            (*it)->currentinfo.node.loc.pos.lvlh = lvlh;
            (*it)->lvlhposition->Init(origineci);
            error = (*it)->Propagate(currentutc, origineci);
            if (error < 0)
            {
                return error;
            }
        ...
        int32_t Simulator::Propagate(double nextutc)
        {
            ...
            for (auto &state : cnodes)
            {
                ...
                iretn = state->Propagate(currentutc, cnodes[0]->currentinfo.node.loc.pos.eci);
            }
            return iretn;
        }
