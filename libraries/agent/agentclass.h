 ...
 * 
 * These functions support the transformation of a generic program into a COSMOS
 * aware program. The first level of transformation creates a COSMOS Client that
 * is able to speak the COSMOS language and communicate with any active COSMOS 
 * Agents, while the second level of transformation creates a full COSMOS Agent.
 * 
 * Clients are COSMOS aware programs that are made aware of the 
 * \ref jsonlib_namespace, and are capable of receiving messages broadcast by 
 * any Agents on the same ::NetworkType. These messages, composed of JSON from 
 * the \ref jsonlib_namespace, contain an initial header containing key 
 * information about the sending Agent, plus any additional 
 * \ref jsonlib_namespace values that the particular Agent cares to make 
 * available. This allows the Clients to collect information about the local 
 * system, and make requests of Agents. COSMOS Clients are equipped with a 
 * background thread that collects COSMOS messages and stores them in a ring. 
 * Reading of messages is accomplised through Cosmos::Agent::readring, which 
 * gives you the next message in the ring until you reach the most recent 
 * message. Ring size defaults to 100 messages, but can by changed with 
 * Cosmos::Agent::resizering. The ring can be flushed at any time with 
 * Cosmos::Agent::clearring. Requests to agents are made with 
 * Cosmos::Agent::send_request. As part of its message collection thread, the 
 * Client also keeps a list of discovered Agents. This list can be used to 
 * provide the Agent information required by Cosmos::Agent::send_request through
 * use of Cosmos::Agent::find_agent. Finally, Clients open a Publication Channel
 * for the sending of messages to other COSMOS aware software. Messages are 
 * broadcast, using whatever mechanism is appropriate for the ::NetworkType 
 * chosen, using Cosmos::Agent::post. They can be assigned any of 256 types, 
 * following the rules of Cosmos::Agent::AgentMessage. The actual content over 
 * the network will be a single type byte, a 2 byte unsigned integer in 
 * little_endian order containing the length in bytes of the header, a JSON 
 * header using values from the \ref jsonlib_namespace to represent meta 
 * information about the Agent, and optional data, either as bytes (if type > 
 * 127), or additional JSON values. The header contains the following fields 
 * from the ::beatstruc, returned from either Cosmos::Agent::readring or 
 ...
 * - ::beatstruc::addr: The appropriate address for the ::NetworkType of the 
 * sending machine.
 ...
 * - ::beatstruc::memory: The memory usage of the machine the Agent is running 
 * on.
 * - ::beatstruc::jitter: The residual jitter, in seconds, of the Agents 
 * heartbeat loop.
 * - ::nodestruc::utcoffset: The offset, in Days, being applied to this times to
 *  time shift it.
 * 
 * Agents are persistent COSMOS aware programs that provide the system framework
 * for any COSMOS implementation. They are similar to UNIX Daemons or Windows 
 * Services in that they run continuously until commanded to stop, and serve a 
 * constant function, either automatically or on demand. In addition to the 
 * features listed for Clients, Agents are provided with two additional 
 * features, implemented as two additional threads of execution.
 * - "Heartbeat": This is a Message, as described above, sent at regular 
 * intervals, with type Agent::AgentMessage::BEAT. The optional data can be 
 * filled with State of Health information, established through 
 * Cosmos::Agent::set_sohstring.
 * - "Requests": Requests are received as plain text commands and arguments, at 
 * the IP Port reported in the Heartbeat. They are processed and any response is
 * sent back. The response, even if empty, always ends with [OK], if understood,
 * or [NOK] if not. Requests and their responses must be less than the size of 
 * the communications buffer. There are a number of requests already built in to
 * the Agent. Additional requests can be added using Cosmos::Agent::add_request,
 * by tieing together user defined functions with user defined ASCII strings. 
 * Built in requests include:
 *   - "forward" - Resends the received request out all interfaces.
 *   - "echo utc crc nbytes bytes" - requests the %Agent to echo the local time 
 * the request was received, the values of the indicated JSON names, the CRC 
 * calculated for the bytes, and the bytes themselves.
 ...
 *   - "monitor" - causes the %Agent to transition to 
 * Cosmos::Agent::State::MONITOR.
 *   - "run" - causes the %Agent to transition to Cosmos::Agent::State::RUN.
 *   - "status" - causes the agent to dump any \ref jsonlib variables it is 
 * monitoring.
 *   - "debug_level" - Returns current Debug Level, or if an argument is 
 * provides, sets it.
 *   - "getvalue {\"json_name_000\"[,\"json_name_001\"]}" - requests the %Agent 
 * to return the values of the indicated JSON names.
 *   - "setvalue {\"json_name_000\":value0[,\"json_name_001\":value1]}" - 
 * requests the %Agent to set
 ...
 *   - "utcstartjson" - return the JSON representing the contents of 
 * utcstart.ini.
 ...
 *   - "heartbeat" - Sends a Heartbeat immediately.
 * 
 * Both Clients and Agents are formed using Cosmos::Agent. Once you have 
 * performed any initializations necessary, you should enter a continuous loop, 
 * protected by Cosmos::Agent::running, and preferably surrendering control 
 * periodically with Cosmos::secondsleep. Upon exiting from this loop, you 
 * should call Cosmos::Agent::shutdown.
 */
    ...
    namespace Support
    {
        ...
        public:
            ...
            enum class State : uint16_t {
                SHUTDOWN=0, /**< Shut down Agent */
                INIT, /**< Agent Initializing */
                IDLE, /**< Do minimal necessary to run */
                RUN, /**< Run without monitoring */
                MONITOR, /**< Run with monitoring */
                ASAFE, /**< Agent in Safe State */
                DEBUG, /**< Agent in Debug State */
                RESET /**< Reset Agent */
            };

            /** @brief Multiple agents per name */
#define AGENT_MULTIPLE true 
            /** @brief Single agent per name */
#define AGENT_SINGLE false
            /** @brief Blocking Agent */
#define AGENT_BLOCKING SOCKET_BLOCKING
            /** @brief Non-blocking Agent */
#define AGENT_NONBLOCKING SOCKET_NONBLOCKING
            /** @brief Talk followed by optional listen (sendto address) */
#define AGENT_TALK SOCKET_TALK
            /** @brief Listen followed by optional talk (recvfrom INADDRANY) */
#define AGENT_LISTEN SOCKET_LISTEN
            /** @brief Communicate socket (sendto followed by recvfrom) */
#define AGENT_COMMUNICATE SOCKET_COMMUNICATE
            /** @brief Talk over multiple interfaces */
#define AGENT_JABBER SOCKET_JABBER

            /** @brief Base AGENT port number */
#define AGENTBASE 10020 
            /** @brief Default SEND port */
#define AGENTSENDPORT 10020
            /** @brief Default RECV port */
#define AGENTRECVPORT 10021
            /** @brief AGENT heartbeat Multicast address */
#define AGENTMCAST "225.1.1.1"
            /** @brief Loopback */
#define AGENTLOOPBACK "127.0.0.1"
            /** @brief Maximum AGENT server list count */
#define AGENTMAXLIST 500
            /** @brief Maximum AGENT heartbeat size */
#define AGENTMAXHEARTBEAT 200
            /** @brief Default AGENT socket RCVTIMEO (100 msec) */
#define AGENTRCVTIMEO 100000
            /** @brief Default minium heartbeat period (10 msec) */
#define AGENT_HEARTBEAT_PERIOD_MIN 0.01

            /** @brief Default size of message ring buffer */
#define MESSAGE_RING_SIZE 100

            /** 
             * @brief Type of Agent Message. Types > 127 are binary. 
             * 
             * @todo Document this.
             */
            enum class AgentMessage : uint8_t {
                ALL=1, /**< All Message types */
                BEAT=2, /**< Heartbeat Messages */
                SOH=3, /**< State of Health Messages */
                GENERIC=4, /**< Generic Messages */
                TIME=5, /**<  */
                LOCATION=6, /**<  */
                TRACK=7, /**<  */
                IMU=8, /**<  */
                EVENT=9, /**< Event Messages */
                REQUEST=10, /**< Request message */
                RESPONSE=11, /**< Response message */
                BINARY=128, /**< >= 128 are binary */
                COMM=129 /**<  */
            };
...
#define MAXARGCOUNT 100
            ...
            /**
             * @ingroup agentclass
             * @defgroup agentclass_typedefs Agent Server and Client Library 
             * typedefs
             * 
             * @{
             */
            ...
            /**
             * @brief List of heartbeats.
             * 
             * Heartbeats for multiple processes found on the multicast bus.
             */
            struct beatstruc_list
            {
                int16_t count; /**< Number of heartbeats in list */
                beatstruc *heartbeat[AGENTMAXLIST]; /**< Pointer to an array of pointers to heartbeats */
            };
...
//            int32_t add_request(string token, no_arg_request_function function, string synopsis="", string description="");
            ...
            /** @brief List of active agents */
            vector <beatstruc> agent_list;
            /** @brief List of active Nodes */
...
//            vector <messstruc> message_ring;
            ...
            /** @brief Last message placed in message ring buffer */
            size_t message_head = MESSAGE_RING_SIZE;
            /** @brief Last message rad in message ring buffer */
            ...
            /**
             * @brief Add implementation of new COSMOS Error
             * 
             * @todo Document this.
             */
...
//            int32_t channel_disable(uint8_t number);
        ...
        private:
            ...
            /** @todo Document this. */
            string hbjstring;
            /** @brief for request thread */
            thread cthread;
            /** @brief Handle for heartbeat thread */
            thread hthread;
            /** @brief Handle for message thread */
            thread mthread;
            /** @brief Last error */
            int32_t error_value;
            /** @brief mutex to protect process_request */
            mutex process_mutex;

            /** 
             * @brief Function in which we generate our time, for the mjd 
             * request. 
             */
            double (*agent_time_producer)() = currentmjd;

            /**
             * @brief Agent Request Entry
             * 
            ...
            {
                string token; /**< Character token for request */
                external_request_function efunction; /**< Pointer to function to call with request string as argument and returning any error */
                /** @todo Remove commented-out code? */
