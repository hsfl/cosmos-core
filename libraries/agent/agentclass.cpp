        ...
        do
        {
            ...
            if (nbytes > 0)
            {
                if (type == AGENT_MESSAGE_ALL || type == input[0])
                {
                    ...
                    // Parse through the message and convert from old json format to new format.
                    size_t jsonConvertFlag = mess.jdata.find("}{");
                    while (jsonConvertFlag!=string::npos)
                    {
                        mess.jdata[jsonConvertFlag]=',';
                        mess.jdata[jsonConvertFlag+1]=' ';
                        jsonConvertFlag=mess.jdata.find("}{", jsonConvertFlag+1);
                    }
