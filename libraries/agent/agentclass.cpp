             ...
             uint16_t dlevel)
{
    ...
    add_request("command", req_command, "node radioout:radioin:repeat command arg1 arg2 arg3 ...",
                ...
                "    EpsSwitchStatus [0-1|vbattbus|5vbus|hdrm|3v3bus|adcs|adcsalt|gps|sband|xband|cce|unibap|ext200|sif]\n"
                "    EpsState {0|1|2|3}\n"
                "    EpsSwitchName {vbattbus|5vbus|hdrm|3v3bus|adcs|adcsalt|gps|sband|xband|cce|unibap|ext200|sif} [0|1|2]\n"
...
void Agent::message_loop() {
    ...
    while (Agent::running()) {
        ...
        if (iretn > 0)
        {
            // Don't process if both proc and adata are empty
            ...
            if (mess.meta.beat.port)
            {
                ...
                {
                    // Have we already seen this realm:node:agent?
                    if (!i.realm.compare(mess.meta.beat.realm) && !i.node.compare(mess.meta.beat.node) && !i.proc.compare(mess.meta.beat.proc))
                    {
                        agent_found = true;
...
int32_t Agent::req_command(string &request, string &response, Agent *agent)
{
    ...
    break;
    case PacketComm::TypeId::CommandObcHeartbeat:
    {
    }
    break;
...
int32_t Agent::post(AgentMessage type, vector <uint8_t> message)
{
    ...
    {
        sprintf((char *)&post[3],"{\"agent_utc\":%.15g,\"agent_realm\":\"%s\",\"agent_node\":\"%s\",\"agent_proc\":\"%s\",\"agent_addr\":\"%s\",\"agent_port\":%u,\"agent_bprd\":%f,\"agent_bsz\":%u,\"agent_cpu\":%f,\"agent_memory\":%f,\"agent_jitter\":%f,\"agent_dcycle\":%f,\"node_utcoffset\":%.15g}",
                cinfo->agent0.beat.utc,
                cinfo->agent0.beat.realm.c_str(),
    ...
    do
    {
        ...
        if (nbytes > 0)
        {
            if (type == Agent::AgentMessage::ALL || type == (AgentMessage)input[0])
            {
                ...
                {
                    string estring;
                    json11::Json jargs = json11::Json::parse(mess.jdata, estring);
                    if (!jargs["agent_utc"].is_null())
                    {
                        mess.meta.beat.utc = jargs["agent_utc"].number_value();
                    }
                    else
                    {
                        continue;
                    }
                    if (!jargs["agent_realm"].is_null())
                    {
                        string realm = jargs["agent_realm"].string_value();
                        if (!realm.empty())
                        {
                            mess.meta.beat.realm = realm;
                        }
                    }
                    if (!jargs["agent_node"].is_null())
                    {
                        string node = jargs["agent_node"].string_value();
                        if (!node.empty())
                        {
                            mess.meta.beat.node = node;
                        }
                    }
                    if (!jargs["agent_proc"].is_null())
                    {
                        string proc = jargs["agent_proc"].string_value();
                        if (!proc.empty())
                        {
                            mess.meta.beat.proc = proc;
                        }
                    }
                    if (!jargs["agent_addr"].is_null())
                    {
                        string addr = jargs["agent_addr"].string_value();
                        if (!addr.empty())
                        {
                            strncpy(mess.meta.beat.addr, addr.c_str(), 17);
                        }
                    }
                    if (!jargs["agent_port"].is_null())
                    {
                        mess.meta.beat.port = jargs["agent_port"].number_value();
                    }
                    if (!jargs["agent_bprd"].is_null())
                    {
                        mess.meta.beat.bprd = jargs["agent_bprd"].number_value();
                    }
                    if (!jargs["agent_bsz"].is_null())
                    {
                        mess.meta.beat.bsz = jargs["agent_bsz"].number_value();
                    }
                    if (!jargs["agent_cpu"].is_null())
                    {
                        mess.meta.beat.cpu = jargs["agent_cpu"].number_value();
                    }
                    if (!jargs["agent_memory"].is_null())
                    {
                        mess.meta.beat.memory = jargs["agent_memory"].number_value();
                    }
                    if (!jargs["agent_jitter"].is_null())
                    {
                        mess.meta.beat.jitter = jargs["agent_jitter"].number_value();
                    }
...
/*! Check the message ring for the requested amount of time. Return as soon as a message of the right type
     * is available, or the timer runs out.
    \param message Vector for storing incoming message.
    \param realm String containing realm name. Empty string means any.
    \param node String containing node name. Empty string means any.
    \param type Type of message to look for, taken from Cosmos::Agent::AgentMessage.
    \param waitsec Number of seconds in timer. If 0, return last message in ring immediatelly.
    \param where One of Where::HEAD or Where::TAIL, indicating whether to start at the head or tail of the ring.
    \return If a message comes in, return its type. If none comes in, return zero, otherwise negative error.
    */
int32_t Agent::readring(messstruc &message, string realm, string node, AgentMessage type, float waitsec, Where where)
{
    if (waitsec < 0.f)
    {
        waitsec = 0.;
