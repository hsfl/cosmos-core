...
*/
    ...
    int32_t Agent::load_node_ids()
    {
        ...
        if (Agent::node_ids.size() == 0)
        {
            ...
            if (fp)
            {
                ...
                fclose(fp);
                Agent::node_ids.resize(max_index+1);
                for (uint16_t i=0; i<tindex.size(); ++i)
                {
                    Agent::node_ids[tindex[i]] = tnodeid[i];
                }
            }
            else
            {
        ...
        return Agent::node_ids.size();
    }

    int32_t Agent::check_node_id(NODE_ID_TYPE node_id)
    {
        int32_t iretn;

        if ((iretn=Agent::load_node_ids()) <= 0)
        {
            return iretn;
        }


        if (node_id > 0 && Agent::node_ids[node_id].size())
        {
            return node_id;
        }
        else
    ...
    int32_t Agent::lookup_node_id(string node_name)
    {
        int32_t iretn;
        ...
        uint8_t node_id = 0;
        for (uint8_t i=1; i<Agent::node_ids.size(); ++i)
        {
            if (Agent::node_ids[i] == node_name)
            {
                node_id = i;
                break;
            }
    ...
    //! \return Node name
    string Agent::lookup_node_id_name(NODE_ID_TYPE node_id)
    {
        string name;
        if (Agent::load_node_ids() > 0 && node_id > 0 && node_id < Agent::node_ids.size() && Agent::node_ids[node_id].size())
        {
            return Agent::node_ids[node_id];
        }
        else
        {
            return "";
