	       ...
	       JSAMPARRAY output_buf, JDIMENSION output_col)
{
  ...
  for (ctr = 0; ctr < 4; ctr++) {
    ...
    tmp0 = ((int32_t) wsptr[0]) << (CONST_BITS+1);
    
    tmp2 = MULTIPLY((int32_t) wsptr[2], FIX_1_847759065)
	 + MULTIPLY((int32_t) wsptr[6], - FIX_0_765366865);
	       ...
	       JSAMPARRAY output_buf, JDIMENSION output_col)
{
  ...
  for (ctr = 0; ctr < 2; ctr++) {
    ...
    tmp10 = ((int32_t) wsptr[0]) << (CONST_BITS+2);
    
    /* Odd part */

    tmp0 = MULTIPLY((int32_t) wsptr[7], - FIX_0_720959822) /* sqrt(2) * (c7-c5+c3-c1) */
	 + MULTIPLY((int32_t) wsptr[5], FIX_0_850430095) /* sqrt(2) * (-c1+c3+c5+c7) */
	 + MULTIPLY((int32_t) wsptr[3], - FIX_1_272758580) /* sqrt(2) * (-c1+c3-c5-c7) */
	 + MULTIPLY((int32_t) wsptr[1], FIX_3_624509785); /* sqrt(2) * (c1+c3+c5+c7) */
	       ...
	       JSAMPARRAY output_buf, JDIMENSION output_col)
{
  ...
  dcval = (int) DESCALE((int32_t) dcval, 3);
