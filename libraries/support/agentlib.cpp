...
cosmosstruc *agent_setup_client(int ntype, string node, uint32_t usectimeo)
{
    ...
    strncpy(cdata[0].agent[0].beat.node, cdata->node.name ,COSMOS_MAX_NAME);
    cdata[0].agent[0].beat.ntype = ntype;
    cdata[1].agent[0].client = 1;
    strncpy(cdata[1].agent[0].beat.node, cdata->node.name ,COSMOS_MAX_NAME);
...
cosmosstruc* agent_setup_server(cosmosstruc* cdata, string name, double bprd, int32_t port, uint32_t bsize, bool multiflag)
{
    ...
    iretn = agent_publish(cdata, cdata->agent[0].beat.ntype, AGENTSENDPORT);
    if (iretn)
    {
...
{
    cdata->agent[0].stateflag = AGENT_STATE_SHUTDOWN;;
    hthread.join();
    cthread.join();
    agent_unsubscribe(cdata);
    agent_unpublish(cdata);
...
int32_t agent_send_request(cosmosstruc *, beatstruc hbeat, const char* request, char* output, uint32_t clen, float waitsec)
{
	...
	if ((iretn=socket_open(&sendchan, SOCKET_TYPE_UDP, hbeat.addr, hbeat.port, AGENT_TALK, AGENT_BLOCKING, AGENTRCVTIMEO)) < 0)
    {
        return (-errno);
    }
...
int32_t agent_set_sohstring(cosmosstruc *cdata, const char *list)
{
    ...
    if (!cdata->agent[0].sohtable.empty())
    {
        cdata->agent[0].sohtable.clear();
    }

    json_table_of_list(cdata->agent[0].sohtable, list, cdata);
    return 0;
}
...
void request_loop(cosmosstruc *cdata)
{
	...
	if ((iretn=socket_open(&((cosmosstruc *)cdata)->agent[0].req,SOCKET_TYPE_UDP,(char *)"",((cosmosstruc *)cdata)->agent[0].beat.port,AGENT_LISTEN,AGENT_BLOCKING,2000000)) < 0)
    {
        return;
    }
...
int32_t agent_publish(cosmosstruc *cdata, uint16_t type, uint16_t port)
{
	...
	case SOCKET_TYPE_UDP:
        ...
        for (uint32_t i=0; i<nif; i++)
        {
            ...
            cdata->agent[0].pub[i].baddr.sin_family = AF_INET;
			if (type == SOCKET_TYPE_MULTICAST)
            {
                sslen = sizeof(ss);
                ...
                cdata->agent[0].pub[cdata->agent[0].ifcnt].baddr.sin_addr = ((struct sockaddr_in *)&ss)->sin_addr;
            }
            else
            {
                ...
                cdata->agent[0].pub[cdata->agent[0].ifcnt].baddr.sin_addr = ((struct sockaddr_in *)&ilist[i].iiAddress)->sin_addr;
                cdata->agent[0].pub[cdata->agent[0].ifcnt].baddr.sin_addr.S_un.S_addr = bcast;
            }
            cdata->agent[0].pub[cdata->agent[0].ifcnt].caddr.sin_port = htons(port);
            cdata->agent[0].pub[cdata->agent[0].ifcnt].baddr.sin_port = htons(port);
        ...
        for (int32_t n=confa.ifc_len/sizeof(struct ifreq); --n >= 0; ifra++)
        {
            ...
            memcpy((char *)&cdata->agent[0].pub[cdata->agent[0].ifcnt].caddr, (char *)&ifra->ifr_addr, sizeof(ifra->ifr_addr));
			...
			if (type == SOCKET_TYPE_MULTICAST)
            {
                ...
                inet_pton(AF_INET,AGENTMCAST,&cdata->agent[0].pub[cdata->agent[0].ifcnt].baddr.sin_addr);
            }
            else
            {
                if ((iretn = setsockopt(cdata->agent[0].pub[cdata->agent[0].ifcnt].cudp,SOL_SOCKET,SO_BROADCAST,(char*)&on,sizeof(on))) < 0)
                ...
                if (ioctl(cdata->agent[0].pub[0].cudp,SIOCGIFBRDADDR,(char *)ifra) < 0)
                {
                    continue;
                }
                cdata->agent[0].pub[cdata->agent[0].ifcnt].baddr = cdata->agent[0].pub[cdata->agent[0].ifcnt].caddr;
                inet_ntop(ifra->ifr_broadaddr.sa_family,&((struct sockaddr_in*)&ifra->ifr_broadaddr)->sin_addr,cdata->agent[0].pub[cdata->agent[0].ifcnt].baddress,sizeof(cdata->agent[0].pub[cdata->agent[0].ifcnt].baddress));
                inet_pton(AF_INET,cdata->agent[0].pub[cdata->agent[0].ifcnt].baddress,&cdata->agent[0].pub[cdata->agent[0].ifcnt].baddr.sin_addr);
            }
            cdata->agent[0].pub[cdata->agent[0].ifcnt].caddr.sin_port = htons(port);
            cdata->agent[0].pub[cdata->agent[0].ifcnt].baddr.sin_port = htons(port);
            cdata->agent[0].pub[cdata->agent[0].ifcnt].type = type;
...
int32_t agent_post(cosmosstruc *cdata, uint8_t type, char *message)
{
    ...
    for (i=0; i<cdata->agent[0].ifcnt; i++)
    {
        ...
        iretn = sendto(cdata->agent[0].pub[i].cudp,(const char *)post,nbytes+mbytes,0,(struct sockaddr *)&cdata->agent[0].pub[i].baddr,sizeof(struct sockaddr_in));
    }
    if (iretn<0)
    {
#ifdef COSMOS_WIN_OS
...
//    ltv.tv_usec = ltv.tv_usec % 1000000;
    ...
    do
    {
        ...
        if (nbytes > 0)
        {
            ...
            {
                if (json_convert_string(json_extract_namedobject(&input[1], "agent_proc")) == cdata->agent[0].beat.proc && json_convert_string(json_extract_namedobject(&input[1], "agent_node")) == cdata->agent[0].beat.node)
                {
                    return 0;
                }
                message = &input[1];
