...
int32_t Beacon::EncodeBinary(TypeId type, cosmosstruc* cinfo, vector<uint8_t> &data)
{
    ...
    case TypeId::CPUBeacon:
        ...
        break;
    case TypeId::CPUNameBeacon:
    {
        uint16_t bindex = 0;
        uint16_t cindex = 0;
        while(cindex < cinfo->devspec.cpu.size() && bindex < data.size())
        {
            uint8_t count = cinfo->devspec.cpu[cindex].name.size();
            data.push_back(count);
            while (count && data.size() < large_beacon)
            {
                data.push_back(cinfo->devspec.cpu[cindex].name[count]);
                --count;
            }
            ++cindex;
        }
    ...
    case TypeId::EPSSWCHBeacon:
        if (cinfo->devspec.swch.size())
        {
            ...
            {
                beacon.swch[i].state = cinfo->devspec.swch[i].state;
    ...
    if (data.size() > 5)
    {
        ...
        if (TypeString.find(type) != TypeString.end())
        {
            ...
            case TypeId::CPUNameBeacon:
            {
                uint16_t bindex = 0;
                uint16_t cindex = 0;
                while(bindex < data.size())
                {
                    uint8_t count = data[bindex++];
                    cinfo->devspec.cpu[cindex].name.resize(0);
                    while (count && bindex < data.size())
                    {
                        cinfo->devspec.cpu[cindex].name.push_back(data[bindex++]);
                        --count;
                    }
                    ++cindex;
                }
            ...
            case TypeId::EPSSWCHBeacon:
            {
                ...
                for (uint16_t i=0; i<(data.size()-5)/COSMOS_SIZEOF(epsswch_beacon); ++i)
                {
                    ...
                    cinfo->devspec.swch[i].state = beacon.swch[i].state;
